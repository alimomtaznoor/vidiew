...
const DropdownList = ({ options, selectedOption, onOptionSelect, triggerElement }: DropdownListProps) => {
  ...
  return (
    <div className="relative">
      ...
      {isOpen && (
        <ul className="absolute bg-white  shadow-lg rounded-xl flex flex-col gap-1 w-auto z-10 top-12 p-1 border border-slate-100">
          {options.map((option) => (
            <li
              key={option}
              className={cn(
                ...
                {
                  "bg-indigo-100 text-white/90": selectedOption === option,
                },
              )}
              onClick={() => handleOptionClick(option)}
            >
              {option}
              {selectedOption === option && <Check className="h-4 w-4 text-indigo-200" />}
            </li>
          ))}
        </ul>
      )}
